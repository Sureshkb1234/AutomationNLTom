...
public class BaseReport {
	...
	public void PrepareConsolidatedReport() {
		...
		try {
			...
			for (int i = 0; i < listOfFiles.length; i++) {
				if (listOfFiles[i].getName().contains(".html") && listOfFiles[i].isFile() && !listOfFiles[i].getName().contains("HTMLEmailableReport") && !listOfFiles[i].getName().contains("index")) {
					String getPassCount = "";
					String getFailCount = "";
							...
							if (line.contains("step-details") && FailedStepFound == true) {
								if (!line.contains("selenium")) {
									if (line.contains("<br>"))
										...
										line = line.replace("<span style=\"font-weight:bold;color:red\">", "");
										line = line.replace("</span></td>", "</td>");
